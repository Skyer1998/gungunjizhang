(function (global, factory) {
	...
	***************************************************************************** */
	...
	var isWx = (typeof wx === "undefined" ? "undefined" : _typeof$1(wx)) === 'object' && typeof wx.getSystemInfoSync === 'function'; // ant miniprogram
	// @ts-ignore

	var isMy = (typeof my === "undefined" ? "undefined" : _typeof$1(my)) === 'object' && typeof my.getSystemInfoSync === 'function'; // in node
	// @ts-ignore

	var isNode = (typeof global === "undefined" ? "undefined" : _typeof$1(global)) && !(typeof window === "undefined" ? "undefined" : _typeof$1(window)); // in browser
	...
	var ImageShape = /*#__PURE__*/function (_Rect) {
	  ...
	  _createClass$1(ImageShape, [{
	    ...
	    key: "draw",
	    value: function draw(context) {
	      var _this = this; // 如果图片还在loading中直接返回，等下次绘制
	...
	var requestAnimationFrame = (typeof window === "undefined" ? "undefined" : _typeof$1(window)) === 'object' && window.requestAnimationFrame ? window.requestAnimationFrame : function (fn) {
	...
	var requestAnimationFrame$1 = (typeof window === "undefined" ? "undefined" : _typeof(window)) === 'object' && window.requestAnimationFrame ? window.requestAnimationFrame : function (fn) {
	  return setTimeout(fn, 16);
	};
	...
	var Timeline$1 = /*#__PURE__*/function () {
	  function Timeline() {
	    ...
	    this.playing = false; // 暂停中

	    this.paused = false; // 暂停的时间点
	  ...
	  _createClass(Timeline, [{
	    ...
	    value: function play(duration, onUpdate, onEnd) {
	      ...
	      this.playing = true;
	      var startTime = clock.now(); // 如果当前正在暂停状态， 从暂停态继续播放
	      ...
	      var play = function play() {
	        ...
	        onUpdate(time);
	        _this.animationFrameNumber = requestAnimationFrame$1(play);
	      };

	      this.animationFrameNumber = requestAnimationFrame$1(play);
	    }
	  }, {
	    key: "pause",
	...
	function jsx(type, config) {
	  var _ref = config || {},
	      key = _ref.key,
	      ref = _ref.ref,
	      props = _objectWithoutProperties(_ref, _excluded); // 保持和automatic模式一致
	...
	parentDirection) {
	  ...
	  node.layout[trailing[crossAxis]] += getTrailingMargin(node, crossAxis) + getRelativePosition$1(node, crossAxis); // Inline immutable values from the target node to avoid excessive method
	  ...
	  if (isMeasureDefined(node)) {
	    var
	    /*bool*/
	    isResolvedRowDimDefined = !isUndefined$1(node.layout[dim[resolvedRowAxis]]);
	    var
	    /*float*/
	    width = CSS_UNDEFINED;
	    ...
	    var
	    /*bool*/
	    isColumnUndefined = !isDimDefined(node, CSS_FLEX_DIRECTION_COLUMN) && isUndefined$1(node.layout[dim[CSS_FLEX_DIRECTION_COLUMN]]); // Let's not measure the text if we already know both dimensions
	  ...
	  var
	  /*bool*/
	  isMainDimDefined = !isUndefined$1(node.layout[dim[mainAxis]]);
	  var
	  /*bool*/
	  isCrossDimDefined = !isUndefined$1(node.layout[dim[crossAxis]]);
	  var
	  /*bool*/
	...
	var ELEMENT_APPEAR = 'appear'; // 标识元素更新

	var ELEMENT_UPDATE = 'update'; // 标识是删除的元素
	...
	var getAnimation = (function (element, animation, nextAttrs, lastAttrs) {
	  if (!animation) return null; // 获取shape的默认属性
	  ...
	  var clipConfig = isFunction(clip) ? clip(element._attrs.attrs) : clip; // 裁剪动画
	...
	function createNodeTree(element, container, px2hd) {
	  ...
	  var attrs = px2hd(props.attrs); // 文本要自动计算文本的宽高, TODO, 后面再优化
	...
	function createElement(node, container, parentLayout, animate) {
	  ...
	  var layout = mergeLayout(parentLayout, originLayout); // 该元素上一次的attrs

	  var lastAttrs = _cache.attrs;

	  var elementAttrs = _objectSpread(_objectSpread(_objectSpread({}, getShapeAttrs(type, layout)), status === ELEMENT_DELETE ? lastAttrs : null), attrs); // 缓存这次新的attrs
	  ...
	  if (type === 'group') {
	    ...
	    var children = renderChildren ? renderChildren : nodeChildren; // 只有group才需要处理children
	...
	function renderJSXElement(element, context, updater) {
	  ...
	  var _element = element,
	      ...
	      _cache = _element$_cache === void 0 ? {} : _element$_cache; // render children first
	  ...
	  if (typeof type === 'function') {
	    ...
	    if (!newElement) return newElement; // recursive render until type is string
	...
	function deleteElement(element) {
	  ...
	  var receiveElement = Children.map(element, function (item) {
	    ...
	    var receiveChildren = deleteElement(children); // 没有子元素，且自身也不需要动画，则直接删除
	...
	function updateElement(nextElement, lastElement) {
	  ...
	  var lastChildren = lastProps.children; // 继续比较子元素

	  var children = compareElement(nextChildren, lastChildren); // 保留缓存值

	  var _cache = mix(_nextCache, _lastCache); // 动画


	  var animation = nextAnimation && nextAnimation.update; // 生成新对象
	...
	function changeTypeToGroup(nextGroupElement, lastShapeElement) {
	  ...
	  var groupChildren = groupProps.children; // let existTransform = false;
	  ...
	  var children = Children.map(groupChildren, function (nextElement) {
	    ...
	    var key = nextElement.key,
	        ...
	        nextProps = nextElement.props,
	        _nextCache = nextElement._cache; // if (nextType === 'group') {
	...
	function compareArray$1(nextElements, lastElements) {
	  ...
	  for (var _i = 0; _i < maxLength; _i++) {
	    ...
	    var _key = nextElement.key; // 有key值定义
	...
	function compareElement(nextElement, lastElement) {
	  ...
	  var lastKey = lastElement.key,
	      lastType = lastElement.type; // key 值不相等
	...
	var Animator = /*#__PURE__*/function () {
	  ...
	  _createClass(Animator, [{
	    ...
	    value: function to(time) {
	      ...
	      this.update(easing(t), time); // 最后一帧
	  ...
	  _createClass(Animation, [{
	    ...
	    value: function createAnimator(element, animation) {
	      var duration = animation.duration,
	          ...
	          onFrame = animation.onFrame; // 校验关键参数
	    ...
	    value: function play(container, onAnimationEnd) {
	      ...
	      var animators = [];
	      var maxDuration = 0;
	      var deleteElements = []; // 遍历整个树，找到全部需要动画的元素
	      ...
	      eachElement(container, function (element) {
	        ...
	        var clip = animation.clip; // 如果有裁剪区动画，处理裁剪区动画
	      ...
	      for (var i = 0, len = deleteElements.length; i < len; i++) {
	        ...
	        var children = element._attrs.children; // 因为group的子元素也有可能有动画，所以这里先把叶子节点删除掉，等动画结束后，再把所有删除的元素删除掉
	...
	function createUpdater(canvas) {
	  ...
	  function process() {
	    var item; // let component;

	    while (item = setStateQueue.shift()) {
	      var _item = item,
	          state = _item.state,
	          component = _item.component,
	          callback = _item.callback; // 如果没有prevState，则将当前的state作为初始的prevState
	    ...
	    canvas.renderComponents(renderComponents); // callback queue

	    commitRenderQueue(); // 清空
	...
	function pickElement(element) {
	  if (!element) return element;
	  return Children.map(element, function (item) {
	    if (!item) return item; // 只需要这几个元素就可以了
	...
	function renderShape(component, children, animate) {
	  var container = component.container,
	      ...
	      componentAnimate = component.animate; // 先清空绘制内容

	  container.clear();
	  animate = isBoolean(animate) ? animate : componentAnimate;
	  var px2hd = context.px2hd;
	  var lastElement = __lastElement || transformFrom && transformFrom.__lastElement; // children 是 shape 的 jsx 结构, component.render() 返回的结构

	  var shapeElement = renderJSXElement$1(children, context, updater); // @ts-ignore

	  component.__lastElement = shapeElement;
	  var renderElement = animate !== false ? compareElement(shapeElement, lastElement) : shapeElement;
	  if (!renderElement) return null; // 生成G的节点树, 存在数组的情况是根节点有变化，之前的树删除，新的树创建
	...
	function setComponentAnimate(child, parent) {
	  var parentAnimate = parent.animate; // 如果父组件不需要动画，子组件全不不执行动画
	...
	function getTransformComponent(component) {
	  if (!component) return null; // @ts-ignore
	...
	function createComponent(parent, element) {
	  ...
	  if (transformFromRef) {
	    var transformFromComponent = transformFromRef ? getTransformFromComponentRef(transformFromRef) : null; // @ts-ignore
	...
	function diff(parent, nextChildren, lastChildren) {
	  ...
	  // render
	  // didMount / didUpdate
	  var childrenArray = []; // 1. 第一轮比较， 直接destroy的元素处理掉，destroy 的元素不需要进入下一阶段
	  ...
	  var shouldProcessChildren = childrenArray.filter(function (element) {
	    var component = element.component,
	        props = element.props; // 说明是新增的元素，需要新建

	    if (!component) return true; // 不需要更新
	  ...
	  renderComponent(shouldRenderComponent); // 按子组件顺序渲染内容
	...
	var Canvas$1 = /*#__PURE__*/function (_Component) {
	  ...
	  function Canvas(props) {
	    ...
	    var theme = px2hd$1(deepMix({}, Theme, customTheme)); // 创建G的canvas
	    ...
	    var updater = createUpdater(_assertThisInitialized(_this)); // 供全局使用的一些变量
	  ...
	  _createClass(Canvas, [{
	    ...
	    value: function resize(width, height) {
	      ...
	      this.canvas.changeSize(width || canvasWidth, height || canvasHeight); // this.canvas.clear();
	      // this.children = null;
	...
	var Base = /*#__PURE__*/function (_Layout) {
	  ...
	  function Base(option) {
	    ...
	    _this = _super.call(this, option); // x y 调换

	    _this.transposed = false; // x，y 的值域，在极坐标中对应的就是弧度和半径
	  ...
	  _createClass(Polar, [{
	    ...
	    value: function update(option) {
	      ...
	      var width = this.width,
	          ...
	          endAngle = _this$endAngle === void 0 ? Math.PI * 3 / 2 : _this$endAngle; // 半径取宽高的最小值

	      var radius = radiusRatio * (Math.min(width, height) / 2); // 极坐标下 x 表示弧度， y 代表 半径
	  ...
	  _createClass(ScaleController, [{
	    ...
	    value: function setScale(field, option) {
	      var options = this.options,
	          scales = this.scales;
	      options[field] = mix({}, options[field], option); // 如果scale有更新，scale 也需要重新创建
	    ...
	    value: function adjustStartZero(scale) {
	      ...
	      var field = scale.field,
	          min = scale.min,
	          max = scale.max;
	      var option = options[field]; // 如果有定义，则不处理
	...
	var Chart = /*#__PURE__*/function (_Component) {
	  ...
	  function Chart(props, context, updater) {
	    ...
	    var _assertThisInitialize = _assertThisInitialized(_this),
	        ...
	        scaleController = _assertThisInitialize.scaleController; // 布局


	    var style = _this.getStyle(props, context);

	    _this.layout = layoutController.create(style); // 坐标系

	    _this.coord = coordController$1.create(coordOption, _this.layout); // scale

	    scaleController.create(scale);
	    _this.data = data; // state
	  ...
	  _createClass(Chart, [{
	    ...
	    value: function willUpdate() {
	      var coordController = this.coordController,
	          props = this.props; // render 时要重置 coord 范围，重置后需要让所有子组件都重新render
	    ...
	    value: function updateCoordFor(component, layout) {
	      ...
	      if (existIndex > -1) {
	        componentsPosition.splice(existIndex, 1, componentPosition); // 先重置，然后整体重新算一次
	    ...
	    value: function getPosition(record) {
	      ...
	      var yScales = this.getYScales(); // default first
	    ...
	    value: function getSnapRecords(point, inCoordRange) {
	      ...
	      if (!geometrys.length) return; // @ts-ignore
	...
	***************************************************************************** */
	...
	var Identity$2 = Identity$1,
	    Linear$2 = Linear$1,
	    Category$2 = Category$1; // 需要映射的属性名

	var ATTRS = ['x', 'y', 'color', 'size', 'shape']; // 分组处理的属性
	...
	var AttrController = /*#__PURE__*/function () {
	  ...
	  _createClass(AttrController, [{
	    ...
	    value: function getAttrOptions(props, justifyContentCenter) {
	      ...
	      var x = options.x,
	          y = options.y;
	      x.justifyContent = justifyContentCenter; // x, y 都是固定Linear 映射
	    ...
	    value: function createAttr(option) {
	      ...
	      var AttrConstructor = scale.isLinear ? Linear$2 : Category$2; // custom Attr Constructor
	...
	var Geometry = /*#__PURE__*/function (_Selection) {
	  ...
	  function Geometry(props, context) {
	    ...
	    _this = _super.call(this, props, context);
	    _this.isGeometry = true; // x 轴居中

	    _this.justifyContent = false; // y 轴是否从0开始

	    _this.startOnZero = false; // 是否连接空值

	    _this.connectNulls = false; // 是否需要排序
	  ...
	  _createClass(Geometry, [{
	    ...
	    value: function _adjustScales() {
	      ...
	      var y = attrs.y;
	      var yField = y.field; // 如果从 0 开始，只调整 y 轴 scale
	    ...
	    value: function _adjustData(records) {
	      var attrs = this.attrs,
	          props = this.props;
	      var adjust = props.adjust; // groupedArray 是二维数组
	    ...
	    value: function _processData() {
	      ...
	      var data = this._saveOrigin(originData); // 根据分类度量进行数据分组


	      var records = this._groupData(data); // 根据adjust分组


	      var dataArray = this._adjustData(records);

	      this.dataArray = dataArray; // scale适配调整，主要是调整 y 轴是否从 0 开始 以及 饼图

	      this._adjustScales(); // 数据排序（非必须）
	      ...
	      for (var i = 0, len = records.length; i < len; i++) {
	        ...
	        for (var j = 0, childrenLen = children.length; j < childrenLen; j++) {
	          ...
	          for (var _k = 0; _k < linearAttrs.length; _k++) {
	            ...
	            var _attr = attrs[_attrName]; // 分类属性的线性映射
	          ...
	          var _coord$convertPoint = coord.convertPoint({
	            ...
	            y: normalized.y
	              ...
	              y = _coord$convertPoint.y; // 获取 shape 的 style
	    ...
	    value: function _getXSnap(invertPointX) {
	      ...
	      var len = values.length; // 如果只有1个点直接返回第1个点
	    ...
	    value: function _getYSnapRecords(invertPointY, records) {
	      ...
	      var yField = yScale.field;
	      var yValue = yScale.invert(invertPointY); // category
	    ...
	    value: function getSnapRecords(point, inCoordRange) {
	      ...
	      var xScale = this.getXScale();
	      var yScale = this.getYScale(); // 如果不在coord坐标范围内，直接返回空
	      ...
	      if (inCoordRange) {
	        ...
	        var yRange = yScale.range; // 如果 inCoordRange=true，当 point 不在 coord 坐标范围内时，调整到 range 内
	      ...
	      var records = this.flatRecords(); // 处理饼图
	    ...
	    _createClass(Line, [{
	      ...
	      value: function mapping() {
	        ...
	        return records.map(function (record) {
	          var children = record.children; // children 有可能为空

	          var _ref = children[0] || {},
	              size = _ref.size,
	              color = _ref.color,
	              shape = _ref.shape,
	              y = _ref.y; // 极坐标时，需加入起点，从而闭合所绘图形
	...
	var withArea = (function (View) {
	  return /*#__PURE__*/function (_withLine) {
	    ...
	    _createClass(Area, [{
	      ...
	      key: "mapping",
	      value: function mapping() {
	        var records = _get$1(_getPrototypeOf(Area.prototype), "mapping", this).call(this); // 坐标轴 y0
	  ...
	  return /*#__PURE__*/function (_Geometry) {
	    ...
	    _createClass(Interval, [{
	      ...
	      value: function getDefaultSize() {
	        ...
	        var size = 1 / values.length * ratio; // 分组时size要除以类别个数
	        ...
	        for (var i = 0, len = records.length; i < len; i++) {
	          ...
	          for (var j = 0, _len = children.length; j < _len; j++) {
	            ...
	            var normalized = child.normalized,
	                mappedSize = child.size; // 没有指定size，则根据数据来计算默认size
	...
	var intervalView = (function (props) {
	  var coord = props.coord;
	  var coordType = coord.type; // 直角坐标系
	...
	var polygonView = (function (props) {
	  var records = props.records,
	      ...
	      LabelView = props.LabelView; // 是否倒置
	    ...
	    _createClass(Axis, [{
	      ...
	      value: function getMaxBBox(ticks, style) {
	        ...
	        ticks.forEach(function (tick) {
	          if (!label) return;
	          var labelStyle = tick.labelStyle,
	              text = tick.text;
	          var bbox = measureText(text, _objectSpread(_objectSpread({}, label), labelStyle));
	      ...
	      value: function getTicks() {
	        ...
	        var field = props.field,
	            chart = props.chart;
	        var scale = chart.getScale(field);
	        var ticks = scale.getTicks(); // 设置tick的样式
	      ...
	      value: function _generateGridPoints(ticks) {
	        ...
	        var dimType = this._getDimType(); // 只需要在 y 的时候生成
	...
	var AxisView = (function (props) {
	  ...
	  var position = props.position; // 直角坐标
	    ...
	    _createClass(Legend, [{
	      ...
	      value: function _init() {
	        ...
	        var _this$getMaxItemBox = this.getMaxItemBox(shape),
	            ...
	            itemMaxHeight = _this$getMaxItemBox.height; // 每行最多的个数


	        var lineMaxCount = Math.floor(width / itemMaxWidth);
	        var itemCount = items.length; // legend item 的行数
	...
	var withGuide = (function (View) {
	  return /*#__PURE__*/function (_Component) {
	    ...
	    function Guide(props) {
	      ...
	      _this = _super.call(this, props); // 创建ref
	    ...
	    _createClass(Guide, [{
	      ...
	      value: function getGuideBBox() {
	        ...
	        var _shape$get2 = shape.get('attrs'),
	            ...
	            height = _shape$get2.height; // getBBox 没有包含 padding 所以这里手动计算 bbox
	      ...
	      value: function parsePoint(record) {
	        ...
	        var chart = props.chart,
	            coord = props.coord;
	        var xScale = chart.getXScales()[0]; // 只取第一个yScale

	        var yScale = chart.getYScales()[0]; // 解析 record 为归一化后的坐标
	...
	var ImageGuideView = (function (props, context) {
	  ...
	  var offsetXNum = context.px2hd(offsetX);
	  var offsetYNum = context.px2hd(offsetY);
	  var posX = x + (offsetXNum || 0) - height / 2;
	  var posY = y + (offsetYNum || 0) - width / 2;
	  return jsx("group", {
	    style: style
	  }, jsx("image", {
	...
	var TagGuideView = (function (props, context) {
	  ...
	  var _getDirect = function _getDirect(point) {
	    ...
	    var horizontal = newDirect[1]; // adjust for vertical direction
	  ...
	  var _getArrowPoints = function _getArrowPoints(direct) {
	    var arrowPoints = []; // const { minX, minY } = guideBBox || {};
	    ...
	    _createClass(Tooltip, [{
	      ...
	      value: function willReceiveProps(nextProps) {
	        ...
	        var _this$props = this.props,
	            ...
	            lastCoord = _this$props.coord; // 默认元素或坐标有变动，均需重新渲染
	      ...
	      value: function _showByData(dataItem) {
	        ...
	        if (!dataItem) return;
	        var props = this.props;
	        var chart = props.chart; // 因为 tooltip 有可能在 geometry 之前，所以需要等 geometry render 完后再执行
	      ...
	      value: function _clearEvents() {
	        ...
	        var _props$triggerOn2 = props.triggerOn,
	            ...
	            triggerOff = _props$triggerOff2 === void 0 ? 'pressend' : _props$triggerOff2; // 解绑事件
	      ...
	      value: function show(point, _ev) {
	        ...
	        var records = snapRecords.map(function (record) {
	          ...
	          var value = yScale.getText(origin[yField]); // 默认取 alias 的配置
	...
	var TooltipView = /*#__PURE__*/function (_Component) {
	  ...
	  _createClass(TooltipView, [{
	    ...
	    value: function _position() {
	      ...
	      var halfWidth = width / 2; // 让 tooltip 限制在 coord 的显示范围内

	      var offsetX = Math.min(Math.max(x - coordLeft - halfWidth, -arrowWidth - radius), coordWidth - width + arrowWidth + radius); // 因为默认是从 coord 的范围内显示的，所以要往上移，移出 coord，避免挡住 geometry
	  ...
	  return /*#__PURE__*/function (_Component) {
	    ...
	    _createClass(Treemap, [{
	      ...
	      value: function treemapLayout() {
	        ...
	        var treemapLayout = treemap() // 默认treemapSquarify
	        .tile(treemapBinary) // .size([1, 1])
	        // @ts-ignore
	        .round(false); // .padding(space)
	        // .paddingInner(space);
	  ...
	  return /*#__PURE__*/function (_Component) {
	    ...
	    _createClass(PieLabel, [{
	      ...
	      value: function getLabels(props) {
	        ...
	        var records = geometry.flatRecords() // 按角度大到小排序
	        .sort(function (a, b) {
	          var angle1 = a.xMax - a.xMin;
	          var angle2 = b.xMax - b.xMin;
	          return angle2 - angle1;
	        }) // 只取前 maxCount 个显示
	        .slice(0, maxCount); // 存储左右 labels
	        ...
	        records.forEach(function (record) {
	          var xMin = record.xMin,
	              xMax = record.xMax,
	              color = record.color,
	              origin = record.origin; // 锚点角度

	          var anchorAngle = getMiddleAngle(xMin, xMax); // 锚点坐标

	          var anchorPoint = getEndPoint(center, anchorAngle, radius + anchorOffset); // 拐点坐标

	          var inflectionPoint = getEndPoint(center, anchorAngle, radius + inflectionOffset); // 锚点方向
	        ...
	        halves.forEach(function (half, index) {
	          var showSide = index === 0 ? 'left' : 'right'; // 顺时针方向排序
	          ...
	          var minY = Math.min.apply(null, pointsY); // 每个 label 占用的高度

	          var labelCount = half.length;
	          var labelHeight = coordHeight / labelCount;
	          var halfLabelHeight = labelHeight / 2; // 线之间的间隔
	          ...
	          if (showSide === 'left') {
	            half.forEach(function (label, index) {
	              ...
	              var endY = coordTop + halfLabelHeight + labelHeight * index; // 文本开始点
	...
	function updateCategoryRange(scale, originScale, range) {
	  ...
	  var valueEnd = end * len; // 保持滑动时个数的稳定

	  var count = Math.round(valueEnd - valueStart);
	  var sliceSatrt = Math.round(valueStart); // 从原始数据里截取需要显示的数据

	  var newValues = originValues.slice(sliceSatrt, sliceSatrt + count); // 根据当前数据的比例，和定义的tickCount计算应该需要多少个ticks

	  var newTickCount = Math.round(tickCount * originValues.length / newValues.length); // 计算新的ticks
	...
	function updateFollow(scales, mainScale, data) {
	  var mainField = mainScale.field,
	      ...
	      mainValues = mainScale.values; // 转成 map 提高查询性能
	...
	var Zoom = /*#__PURE__*/function (_Component) {
	  ...
	  function Zoom(props) {
	    ...
	    _this.onPan = function (ev) {
	      var _this$props$mode = _this.props.mode,
	          mode = _this$props$mode === void 0 ? 'x' : _this$props$mode;
	    ...
	    _this.onPinch = function (ev) {
	      var _this$props$mode2 = _this.props.mode,
	          mode = _this$props$mode2 === void 0 ? 'x' : _this$props$mode2;
	  ...
	  _createClass(Zoom, [{
	    ...
	    value: function willMount() {
	      ...
	      var values = scale.values;
	      this.scale = scale;
	      this.originScale = cloneScale$1(scale); // 图表上最少显示 MIN_COUNT 个数据
	    ...
	    value: function _doYPan(ev) {
	      ...
	      var ratio = -deltaY / coordHeight;

	      this._doPan(ratio);
	    }
	  }, {
	    key: "_doPan",
	    value: function _doPan(ratio) {
	      var startRange = this.startRange;

	      var _startRange = _slicedToArray(startRange, 2),
	          start = _startRange[0],
	          end = _startRange[1];
	    ...
	    value: function _doXPinch(ev) {
	      ...
	      var rightLen = Math.abs(right - center.x); // 计算左右缩放的比例
	    ...
	    value: function _doYPinch(ev) {
	      ...
	      var coordHeight = coord.height,
	          top = coord.top,
	          bottom = coord.bottom;
	      var topLen = Math.abs(center.x - top);
	      var bottomLen = Math.abs(bottom - center.x); // 计算左右缩放的比例
	    ...
	    key: "_doPinch",
	    value: function _doPinch(startRatio, endRatio, zoom) {
	      var startRange = this.startRange,
	          minScale = this.minScale;

	      var _startRange2 = _slicedToArray(startRange, 2),
	          start = _startRange2[0],
	          end = _startRange2[1];
	    ...
	    key: "updateRange",
	    value: function updateRange$1(range) {
	      if (!range) return;
	      ...
	      var rangeLength = end - start; // 处理边界值
	      ...
	      var props = this.props,
	          scale = this.scale,
	          originScale = this.originScale; // 更新主 scale
	      ...
	      if (autoFit) {
	        var followScale = this._getFollowScales();
	    ...
	    value: function _getScale() {
	      var _this$props = this.props,
	          _this$props$mode3 = _this$props.mode,
	          mode = _this$props$mode3 === void 0 ? 'x' : _this$props$mode3,
	    ...
	    value: function _getFollowScales() {
	      var _this$props2 = this.props,
	          _this$props2$mode = _this$props2.mode,
	          mode = _this$props2$mode === void 0 ? 'x' : _this$props2$mode,
